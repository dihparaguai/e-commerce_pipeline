...
def ingest_vendas() -> None:
    """
    Job PySpark para ler a base de vendas brutos (CSV) e gravá-la na camada Bronze no MinIO
    como Parquet particionado pelas colunas: data_pedido, estado e status_pedido.
    """
    ...
    try:
        ...
        raw_csv_path = "/opt/airflow/data/raw/vendas.csv"
        ...
        try:
            ...
            df_new_unique = df.dropDuplicates(["pedido_id"])
            
            # Mantém apenas os registros novos que não existem no histórico
            df_to_append = df_new_unique.join(df_existing, on="pedido_id", how="left_anti")
            ...
            df_to_append = df.dropDuplicates(["pedido_id"])
        ...
        (
            df_to_append.write
            .mode("append")
            .partitionBy("data_pedido", "estado", "status_pedido")
