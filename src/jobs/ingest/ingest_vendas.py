...
def ingest_vendas() -> None:
    """
    Job PySpark para ler novos arquivos de vendas (CSV) e gravá-los na camada Bronze no MinIO com controle de CDC local (evitando duplicar arquivos já processados) e de chaves.
    """
    logger.info("Iniciando a ingestão de vendas (Raw -> Bronze)...")
    raw_dir = "/opt/airflow/data/raw/vendas"
    log_dir = "/opt/airflow/data/cdc"
    log_filename = "vendas.csv"
    bronze_parquet_path = "s3a://bronze/vendas"
    
    new_files = ingest.get_new_files(raw_dir, log_dir, log_filename)
    
    if not new_files:
        logger.info("Nenhum arquivo novo de vendas encontrado para ingestão.")
        return
    logger.info("Arquivos novos de vendas detectados para processamento: {}", new_files)
    
    # Inicializa a sessão do Spark
    spark = get_spark_session("IngestionVendasBronze")
    try:
        ...
        df_to_append = ingest.deduplicate_and_filter_existing(spark, df, "pedido_id", bronze_parquet_path)
        ...
        logger.info("Ingestão de vendas finalizada com sucesso!")
        
    except Exception as e:
        logger.error("Falha durante o processamento de ingestão de vendas: {}", str(e))
        raise e
