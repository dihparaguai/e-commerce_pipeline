...
def ingest_estoque() -> None:
    """
    Job PySpark para ler novos arquivos de estoque (CSV) e gravá-los na camada Bronze no MinIO com controle de CDC local (evitando duplicar arquivos já processados) e de chaves.
    """
    logger.info("Iniciando a ingestão de estoque (Raw -> Bronze)...")
    raw_dir = "/opt/airflow/data/raw/estoque"
    log_dir = "/opt/airflow/data/cdc"
    log_filename = "estoque.csv"
    bronze_parquet_path = "s3a://bronze/estoque"
    
    new_files = ingest.get_new_files(raw_dir, log_dir, log_filename)
    
    if not new_files:
        logger.info("Nenhum arquivo novo de estoque encontrado para ingestão.")
        return
    logger.info("Arquivos novos de estoque detectados para processamento: {}", new_files)
    ...
    try:
        ...
        df_to_append = ingest.deduplicate_and_filter_existing(spark, df, "produto_id", bronze_parquet_path)
